...
new #[Layout('layouts.app')] #[Title('WM001 Lab Scales Daily Calibration')] class extends Component {
    ...
    {
        $this->wm001_date = now()->toDateString();
        $this->wm001_operator_name = (string) (auth()->user()?->name ?? '');
    }

    #[Computed]
    public function doneToday(): bool
    {
        return LabScaleCalibration::query()->whereDate('checked_date', now()->toDateString())->exists();
    }
        ...
        <div style="background:#fff;border:1px solid #dbe1ea;border-radius:18px;overflow:hidden;box-shadow:0 10px 30px rgba(15,23,42,0.08);">
            <div style="padding:24px 26px;background:linear-gradient(135deg,#f8fafc 0%,#e0ecff 100%);border-bottom:1px solid #dbe1ea;">
                <div style="display:flex;align-items:center;gap:18px;flex-wrap:wrap;">
                    <div>
                        <div style="font-size:1.3rem;font-weight:900;color:#1a1a2e;letter-spacing:-0.02em;line-height:1;">WM001 LAB SCALES DAILY CALIBRATION</div>
                        <div style="font-size:0.75rem;font-weight:700;color:#64748b;letter-spacing:.14em;margin-top:4px;">TARGET 100 &plusmn; 0.02</div>
                    </div>
